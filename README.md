...
| **小珠光图像对比 / 图像对比-化神级** | 两张图像并排对比查看，预览区可拖动节点；化神级支持棋盘格、红底、蓝底与自定义透明背景 |
...
| **小珠光音频保存** | 保存音频到文件 |
| **🤖 大模型节点** | |
| **小珠光提示词** | MiniMax-H3 / Qwen-Image-2.1 专用提示词优化；不含自定义 Skill；支持中英文输出 |
| **小珠光提示词-化神级** | 提示词优化节点，包含 Skill 预设管理（编辑、导入 TXT/MD、全局云端持久化及配置备份恢复） |
...
│   ├── xzg_h3_prompt.py         # 通用提示词优化节点（H3 / Qwen-Image-2.1）
...
- **提示词节点拆分**：新增「小珠光提示词」（不含自定义 Skill）与「小珠光提示词-化神级」（含 Skill 预设管理：编辑、导入 TXT/MD、全局云端持久化及备份恢复）两个独立节点；统一支持 MiniMax-H3 / Qwen-Image-2.1 / 通用 Skill，支持中英文独立输出。
- **图像对比-化神级**：新增棋盘格、红底、蓝底与自定义透明背景四种检视模式；图片加载器增强并更名为「小珠光图片加载器-化神级」。
- **uiltnode 徽章**：新增前端脚本隐藏加密工作流包装器的来源名称徽章。
- **达芬奇桥接增强**：播放头时间码捕获/恢复、渲染导出、音频渲染、时间线源匹配等逻辑完善。
- **音频加载/保存节点优化**。
- 版本号：`pyproject.toml` / `extension.json` 升至 `20.0.0`；推送 `v20.0.0` 标签自动创建 GitHub Release 并发布到 Comfy Registry。
