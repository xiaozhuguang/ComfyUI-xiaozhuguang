...
| ⌨️ **自定义快捷键** | 后端持久化快捷键配置（换浏览器不丢失）、执行全图/选中节点/编组内节点、Ctrl+G 官方/小珠光编组选择、主题导出导入含快捷键 |
| 🕶️ **隐藏菜单** | 任意菜单项上鼠标中键点击直接隐藏该菜单项；主题面板「菜单隐藏」页集中查看 / 一键恢复、含「使用说明」标签；配置持久化到后端，云端换浏览器不丢失 |
| 📦 **配置云持久化** | 收藏/工作流元数据/快捷键/菜单隐藏/面板宽度等写入后端存储，云端环境跨会话、跨浏览器共享（`/xzg_cloud_store`） |
| 🖼 **图像节点** | 图像加载器（拖入上传/多图模式）、图像保存（懒编码+化神级自定义输出）、图像对比、分割/合并、智能裁剪 ATBC、图像回贴 ATR、人脸对齐 Face Align、图像预览 |
| 🎬 **视频节点** | 视频加载器（1GB 分块上传 + 并发重试 + 4K 高清预览）、视频信息读取、帧提取、首尾帧、帧优化、合并视频 |
...
- ComfyUI 左下角设置 → xiaozhuguang → 点击「配置快捷键」按钮
- 支持捕获按键组合（Ctrl/Shift/Alt/Meta + 任意键）
- 支持添加、删除、保存到后端
...
| 动作 | 说明 |
|------|------|
| **执行全图** | 执行整个工作流（等效 Ctrl+Enter） |
| **执行选中节点** | 执行选中节点及其上游依赖，无选中时执行全图 |
| **执行编组内节点** | 执行鼠标所在编组内的节点，支持小珠光编组和原生编组 |

### Ctrl+G 编组选择

按 `Ctrl+G` 弹出选项菜单，可选择创建：

- **官方编组**：ComfyUI 原生 LGraphGroup
- **小珠光编组**：支持折叠、锁定、组内执行、动画等增强功能

点击空白处或按 Esc 关闭菜单。

### 备份恢复

快捷键配置随主题导出/导入一起备份，可跨机器迁移。
...
- 新建工作流偶发出现上一工作流图形：configure 阶段立即提交切换，杜绝前端序列化残留
- 关闭未保存工作流后新建同名 tab 旧箭头重现：关闭 / 删除时清理内存缓存与 localStorage 备份
- 已保存工作流重开绘图丢失：关闭仅清临时、保存保留、重开恢复备份
- 绘图显示时机：切换 tab 快速显示；新打开工作流延迟 3 秒等画布节点 / 视图完全加载稳定后再显示，杜绝"先错后对"缩放

**🏷️ 版本号 / 发布**

- 版本号：`pyproject.toml` / `extension.json` 升至 `14.0.11`，推送 `v14.0.11` tag 触发 GitHub Actions 自动创建 Release + 发布到 Comfy Registry

### V14.0.10 (2026-09-01)

**⌨️ 快捷键：修复 D / F 裸键在输入框内无法输入**

- 修复前：匹配到已配置快捷键（含默认 D、F 裸键）即无条件 `preventDefault()`，输入框 / 提示词框内打不出 D / F
- 修复后：输入框 / 文本域 / 可编辑元素内的裸键完全放行，正常输入；带修饰键组合（如 Ctrl+D）仍屏蔽浏览器默认收藏行为但不触发动作；画布上 D / F / Ctrl+D 快捷键行为不变

**🏷️ 版本号 / 发布**

- 版本号：`pyproject.toml` / `extension.json` 升至 `14.0.10`，推送 `v14.0.10` tag 触发 GitHub Actions 自动创建 Release + 发布到 Comfy Registry
...
- 分辨率后缀例外：`720分辨率` → `七二零分辨率`（保持按位读）

**🕶️ 菜单隐藏面板：新增「使用说明」标签页**

- 画布菜单 / 节点菜单标签右侧新增「使用说明」，5 步说明鼠标中键隐藏与恢复操作
- 样式优化：字号 12px、行高 1.8、无外高亮框、撑满面板不留白不滚动

**⌨️ 快捷键：屏蔽浏览器默认 Ctrl+D（收藏当前页）**

- 误触不再弹出收藏框；设置对话框内可正常捕获 / 添加快捷键；输入框内仅拦截默认行为、不触发动作

**🏷️ 版本号 / 发布**

- 版本号：`pyproject.toml` / `extension.json` 升至 `14.0.4`，推送 `v14.0.4` tag 触发 GitHub Actions 自动创建 Release + 发布到 Comfy Registry

### V14.0.3 (2026-08-30)

**🏷️ 版本号 / 发布**

- 版本号：`pyproject.toml` / `extension.json` 升至 `14.0.3`，重新发布以递增 Comfy Registry 版本编号（代码与 14.0.1 一致）

### V14.0.2 (2026-08-30)

**🏷️ 版本号 / 发布**

- 版本号：`pyproject.toml` / `extension.json` 升至 `14.0.2`，重新发布以递增 Comfy Registry 版本编号（代码与 14.0.1 一致）

### V14.0.0 (2026-08-30)

**⌨️ 自定义快捷键系统（后端持久化）**

- 新增 `web/xzg_shortcuts.js`，配置存储于插件目录 `xzg_shortcuts.json`，换浏览器/换服务器不丢失
- 后端 `GET/POST /xzg/shortcuts` 路由，设置面板「配置快捷键」按钮，支持捕获按键、选择动作、增删保存
- 支持动作：执行全图、执行选中节点、执行鼠标所在编组
- 主题导出/导入包含快捷键配置，可跨机器备份恢复

**⌨️ D 键执行选中节点（自研，替代 rgthree）**

- 移除对 rgthree 插件的依赖及硬编码 D 键，小珠光自行实现
- 选中非静音节点时执行节点及上游依赖，无选中时回退执行全图
- 修复种子随机化（control_after_generate）正常生效
- 修复节点校验断链（字符串 ID 连接、嵌套数组连接正确识别）
- 移除死代码：`getOutputNodes`、`recursiveAddQueueNodes`、`queueSelectedOutputNodes`

**⌨️ F 键执行编组内节点（支持双编组）**

- 移除硬编码 F 键，迁移至自定义快捷键系统
- 支持小珠光编组和 ComfyUI 原生编组，通过画布坐标检测鼠标所在编组

**⌨️ Ctrl+G 编组类型选择**

- 按 Ctrl+G 弹出选项菜单，可选择创建「官方编组」或「小珠光编组」
- 不再吞掉官方编组快捷键，菜单屏幕中央显示，点击空白处或 Esc 关闭
...
- 版本号：`pyproject.toml` 从 `13.0.4` 升至 `14.0.0`
- Release / Registry：推送 `v14.0.0` tag 时自动创建 GitHub Release + 发布到 Comfy Registry
