...
| 操作 | 方式 |
|------|------|
| **打开工作流** | 单击工作流项，加载到**当前画布**并激活对应官方工作流（已打开的仅切换、不重复加载）。也可点面板顶部「📖 说明」按钮查看完整使用说明 |
| **拖拽到画布** | 将工作流项拖拽到画布空白处，直接导入节点（不增加使用频率） |
| **定位分类** | 单击工作流项左侧的四个圆点图标，左侧分类树自动展开并高亮其所属分类 |
| **重命名** | 右键工作流项 → 选择「重命名」（保留手工加的编号前缀） |
| **移动分类** | 右键工作流项 → 「移动到分类」选择目标文件夹；或直接拖拽工作流项到分类文件夹 |
| **删除** | 右键工作流项 → 选择「删除」（移入回收站，可恢复） |
| **保存工作流** | 使用 ComfyUI 官方保存（顶栏按钮或 `Ctrl+S`）。保存后本面板会自动同步显示 |

### 分类管理

- 支持**多级嵌套文件夹**分类
- 右键「全部」或分类项 → 选择「新建分类」/「新建子分类」
- 工作流可拖拽到分类文件夹中移动
- 右键分类可删除文件夹（含其中的工作流）
- 「根目录未分类」显示未归类的工作流
