...
- **小珠光 QWEN 提示词**（`nodes/xzg_h3_prompt.py`）
  - 新增英文提示词、中文提示词两个独立输出端口；支持仅英文、仅中文与中英双语三种输出模式
  - 中英双语模式先生成英文提示词，再将其精确翻译为中文，保证字段、时间点、标签、数字与约束一致
  - Qwen-Image-2.1 不再把画幅比例交给提示词推理；强化人物/物体与环境的空间关系、环境光、投影、反射光、漫反射与材质反光描述

- **小珠光图像加载器**（`web/xzg_image_loader.js`）
  - 多图缩略图右键菜单新增追加图片、替换图片、清除其它；空白区域右键也可直接追加图片
  - 新增图片统一追加到列表末尾，覆盖上传、双击、拖入、粘贴与文件列表选择等入口

- **小珠光图像对比**（`web/xzg_image_compare.js`）
  - 预览区域支持拖动节点；顶部功能按钮与批次标签仍保持可点击

### 变更

- `pyproject.toml` / `extension.json` 版本提升至 `19.0.0`，推送 `v19.0.0` 标签触发 GitHub Release 与 Comfy Registry 发布
...
- **小珠光视频对比（同步预览）高码率优化**（`web/xzg_video_sync_preview.js` + `web/xzg_video_player.js` + `nodes/xzg_video_loader.py`）
  - 双路/多路对比按数量限制预览解码尺寸（1440p / 1080p / 720p）并缩减至合适的预读帧数，降低高码率原片并发解码压力
  - 高于 10 Mbps 的视频自动生成可复用 H.264/AAC 对比代理（视频 9.5 Mbps + 音频 128 kbps），原视频和节点输出不受影响
  - 增加播放缓冲状态提示；同步校准改为持续明显漂移才 seek，避免短暂掉帧反复抢占解码器
  - 关闭对比窗口主动清理 Canvas、帧缓存和迟到完成的独占解码器，避免内存残留
  - 新增「带鱼屏」布局：双路竖屏视频可居中紧邻显示，避免超宽屏左右间距过大

### 变更

- `pyproject.toml` / `extension.json` 版本提升至 `18.0.7`，推送 `v18.0.7` 标签触发 GitHub Release 与 Comfy Registry 发布
...
- **「小珠光视频保存-化神级」自定义输出目录**（`nodes/xzg_video_combine.py` + `nodes/xzg_video_save_davinci.py` + `web/xzg_video_combine.js` + `web/xzg_video_save_davinci.js`）
  - 预览区新增第三个悬浮按钮「⚙ 输出设置」（与「导出到达芬奇 / 发送到快剪」同款悬浮文字按钮），点击弹出设置弹窗，交互与「小珠光图像保存-自定义输出」同一套约定
  - 弹窗内：「默认输出」开关（默认开，保持旧行为）+「输出目录」输入框 +「浏览…」目录选择器（复用图像节点同款文件夹浏览器）+ 模板变量提示
  - base_dir 支持：绝对路径直接使用（如 `D:\素材\达芬奇`）/ 相对路径拼到 ComfyUI output/ 下 / `{date}` `{time}` `{datetime}` `{workflow}` `{node_id}` `{format}` 模板变量；路径非法字符自动替换为 `_`（保留盘符与分隔符）
  - 自定义绝对路径输出：文件在 output/ 之外，/view 无法服务 → 保存时注册会话级预览令牌，前端经新路由 `/xzg/davinci/view-abs` 拉流预览（支持 Range，令牌守卫不开放任意文件读取）；ComfyUI 重启后令牌失效，需重新运行节点
  - 达芬奇导入（手动「导出到达芬奇」按钮与「自动导出到达芬奇」开关）均支持自定义路径：绝对路径走令牌解析，相对路径并入 subfolder 后按 output 解析，`_resolve_abs_path` 优先采用保存时写入的 abs_path
  - 「发送到快剪」对绝对路径输出给出明确提示（快剪媒体库仅支持 ComfyUI 目录内文件）；旧工作流参数按位置恢复不受影响（新参数追加在 required 末尾，前端隐藏 widget 仍参与序列化）
