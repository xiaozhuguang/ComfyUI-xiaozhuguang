...
转换策略（策略A：量词/单位 白名单匹配）：
  ...
  3) 其他场景（纯数字、代码、分辨率如 1280x720、串号等）按位逐字符转换：
       12    → 一二
       1280  → 一二八零
  4) 映射：0→零, 1→一, 2→二, 3→三, 4→四, 5→五, 6→六, 7→七, 8→八, 9→九
  5) 仅识别替换 0-9、小数点；其他字符（中文/英文/标点/负号等）保持原样
"""
            ...
            "required": {
                "text": ("STRING", {
                    ...
                    "placeholder":
                        "【小珠光文本框 使用说明】\n"
                        "双通道输出：\n"
                        "  · text        → 原始文本（原样输出，便于其他节点复用）\n"
                        "  · text_zh_num → 数字转中文后的文本（默认始终开启）\n"
                        "\n"
                        "数字转中文规则：\n"
                        "1) 乘积 / 分辨率（数字 × 数字）— 按位读 + 乘以 连接\n"
                        "     1280x720   → 一二八零乘以七二零\n"
                        "     1920×1080  → 一九二零乘以一零八零\n"
                        "     1x2x3      → 一乘以二乘以三\n"
                        "\n"
                        "2) 数字 + 量词 / 单位（白名单）— 完整读数，英文缩写自动转中文\n"
                        "     计数：1个女孩 → 一个女孩；12只猫 → 十二只猫\n"
                        "     年龄：12岁 → 十二岁；105周岁 → 一百零五周岁\n"
                        "     长度：188cm → 一百八十八厘米；5km → 五千米\n"
                        "     重量：75kg → 七十五千克；3.14kg → 三点一四千克\n"
                        "     温度：36.5℃ → 三十六点五摄氏度\n"
                        "     电学：220V → 二百二十伏\n"
                        "     容量：1.5L → 一点五升；500mL → 五百毫升\n"
                        "     货币：12元 → 十二元\n"
                        "     时间：3小时 → 三小时；30分钟 → 三十分钟\n"
                        "     百分：12% → 百分之十二；12.5% → 百分之十二点五\n"
                        "\n"
                        "3) 第 / 其 前缀（序数）— 完整读数 + 可选后缀\n"
                        "     第12章 → 第十二章；第1名 → 第一名；第3次 → 第三次\n"
                        "     第2版 → 第二版；第5届 → 第五届；第10层 → 第十层\n"
                        "\n"
                        "4) 其他场景（纯编号 / 代码 / 串号）— 按位读\n"
                        "     12 → 一二；1280 → 一二八零；型号1234 → 型号一二三四\n"
                        "\n"
                        "支持乘号：x  X  ×  *（前后可带空格）\n"
                        "提示：数字后跟白名单内的量词/单位，才会按完整读数读出，否则按位读。",
                }),
            },
        }
